...
```sql
SELECT crew_id, COUNT(*) AS attended_days
FROM attendance
GROUP BY crew_id;
```
...
```sql
SELECT crew_id, COUNT(start_time) AS start_time_recorded_days
FROM attendance
GROUP BY crew_id;
```
...
```sql
SELECT crew_id, MIN(start_time) AS earliest_start, MAX(start_time) AS latest_start
FROM attendance
GROUP BY crew_id;
```
